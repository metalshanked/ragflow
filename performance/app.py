    ...
    <aside class=\"sidebar\">
      ...
      <section class=\"panel\">
        ...
        <form id=\"run-form\" class=\"form-grid\">
          ...
          <label>Parallel Runs<input name=\"parallel_runs\" type=\"number\" min=\"1\" max=\"50\" value=\"1\" /><span class=\"subtle\">Parallel runs are the number of simultaneous datasets/runs. If this exceeds Total Runs, Total Runs is raised to match.</span></label>
...
function syncRunCounts() { const form = document.getElementById('run-form'); const totalField = form.elements.namedItem('run_count'); const parallelField = form.elements.namedItem('parallel_runs'); if (!totalField || !parallelField) return; const total = Math.max(1, Number(totalField.value) || 1); const parallel = Math.max(1, Number(parallelField.value) || 1); if (parallel > total) totalField.value = String(parallel); }
...
document.getElementById('run-form').addEventListener('input', (event) => { if (event.target?.name === 'run_count' || event.target?.name === 'parallel_runs') syncRunCounts(); persistFormState(); });
document.getElementById('run-form').addEventListener('change', (event) => { if (event.target?.name === 'run_count' || event.target?.name === 'parallel_runs') syncRunCounts(); persistFormState(); });
...
async def start_run() -> Any:
    ...
    parallel_runs = max(1, safe_int(form.get("parallel_runs"), 1))
    if parallel_runs > run_count:
        run_count = parallel_runs
